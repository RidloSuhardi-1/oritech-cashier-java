...
public class KasirController {
    ...
    private void process() {
        try {
            ...
            view.getLblCurrentId().setText("ID Saat ini : " + getId());
    ...
    private void reset() {
        ...
        JLabel currentId = view.getLblCurrentId();
        int dialogButton = JOptionPane.showConfirmDialog(null, "Anda yakin ingin mereset tabel? hal ini akan mengembalikan ID ke posisi awal (0).");
        
        switch(dialogButton) {
            case JOptionPane.YES_OPTION:
                setId(0);
                inTable.setText("");
                currentId.setText("ID Saat ini : " + getId());
                JOptionPane.showMessageDialog(null, "Tabel dan ID berhasil direset");
                break;
    ...
    private void openDatabase() {
        ...
        if (JFileChooser.APPROVE_OPTION == loadFile.showOpenDialog(view))
        {
            ...
            try {
                ...
                char charter[] = new char[doc.getLength()];
                int i = 0;
                ...
                while ((decimal = lineReader.read()) != -1) {
                   ...
                   // memasukkan semua karakter ascii
                   charter[i] = ascii;
                   i++;
                }
                
                for (int k = 0; k < charter.length; k++) {
                    // membandingkan nilai ascii yang disimpan dengan nilai berikutnya
                    if ((charter[k] == '[') && (String.valueOf(charter[k+1]).matches(regex))) {
                        System.out.println("ID Sebelumnya " + getId());
                        id++;
                        System.out.println("ID Saat ini " + getId());
                        System.out.println("");
                    }
                }
        ...
        if (!(inFind.getText().isEmpty()) || !(inFind.getText().equals("")))
        {
            try {
                ...
                if (gotcha) {
                    ...
                    JOptionPane.showMessageDialog(null, "Maaf data ID " + inFind.getText() + " tidak ditemukan");
                }
